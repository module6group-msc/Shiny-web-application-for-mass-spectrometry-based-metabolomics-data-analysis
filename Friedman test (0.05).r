...
Friedman_0.05 <- paste("Friedman_0.05") #set the name
dir.create(Friedman_0.05) #create directory
setwd(Friedman_0.05) #set the new directory as working directory
...
for (m in 3:nrow(new_data)) { #for every metabolite for the whole datafile of normalized metabolites
  ...
  if (sum_model["p.value"]<0.05) { # check if this particular metabolite significant p-value
...
for (o in 1:nrow(new_sign)) { #for every significant metabolite (for every row in new_sign dataframe ---- remember the new_sign dataframe contains the idx for each metabolite (the number of row in data dataframe))
  ...
  #reverse the dataframe, in order to have the data in columns
  metabolite_original_rev=t(metabolite_original)
    # t() command creates table
  
  metabolite_original_rev=as.data.frame(metabolite_original_rev) #make it a dataframe
  ...
  #We need to transform the scores because we have too big values. Otherwise, the graphs do not seem to be informative.
  ...
  names(trans_data)[3] <- "LogScores" #rename the column with the log10 scores
  ...
  #make the graph
  boxplot(trans_data$Logscore$Timepoint, 
          main=mytitle2, 
  ...
  #Actually, it returns to the MAIN working directory which is the "Friedman_0.05" in order to make a new directory for the next metabolite
}
